...
进入 XanMod 内核后，Speed Slayer 原生 TCP 调优会按前台阶段执行并同步写入 `/etc/vps-argo-vmess/tcp-optimize.log`：

1. 检测虚拟内存（SWAP）配置
2. 智能测速/检测服务器带宽并计算最优缓冲区
3. 清理 `/etc/sysctl.conf` 和 `/etc/sysctl.d` 中的 TCP buffer 冲突
4. 创建 Speed Slayer 独立 sysctl 配置文件
5. 应用所有优化参数
6. 应用 FQ 队列、limits、DNS、IPv6 策略并验证结果

可通过环境变量指定带宽/地区，避免默认测速阻塞安装：

```bash
SPEED_BANDWIDTH_MBPS=2000 SPEED_REGION=global speed --optimize
SPEED_AUTO_SPEEDTEST=0 speed --optimize   # 跳过自动测速，使用默认/手动带宽

输出中的 `Bandwidth Source` 含义：

- `measured`：Ookla Speedtest Upload 实测值
- `manual`：由 `SPEED_BANDWIDTH_MBPS` 手动指定
- `default`：测速失败或关闭测速后使用默认 1000Mbps
```

## 重启确认

首次安装内核组件成功后，脚本会直接询问是否现在重启，默认回车为 `Y`。确认后自动执行重启；重启后只需输入 `speed`，会自动续跑后续 TCP 调优与 Argo 节点安装。
...
```text
Speed Slayer · 控制台
GitHub: https://github.com/cshaizhihao/speed-slayer
1. 一键执行完整流程
2. 节点管理
3. TCP 加速
4. 诊断与日志
