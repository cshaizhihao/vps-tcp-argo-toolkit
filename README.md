...
- 优化所有交互子菜单：执行查看状态、查看节点、诊断、日志、修复等操作后不再直接退出脚本，而是按回车返回当前子菜单。
- 所有子菜单统一支持 `0` 返回主菜单，主菜单继续支持 `0` 退出。
- 查看节点/订阅信息时，`vmess://` 链接会用高亮颜色显示，订阅 URL 也会突出显示。

---

## V2.0.6 修复说明

- 修复续跑状态不区分流程的问题：单独 TCP 调优重启后只继续 TCP，不会自动部署 Argo。
- 恢复重启后输入 `speed` 自动续跑当前流程；无续跑状态时仍进入主菜单。
- XanMod 源增加 `bookworm` / `trixie` / `releases` 自动探测，解决部分系统源里没有内核包的问题。
- XanMod 安装阶段增加更细的百分比进度提示，长时间等待时更容易判断执行位置。

---

## V2.0.5 修复说明

- 修复安装入口不带参数时只运行临时脚本、没有安装 `/usr/local/bin/speed` 的问题。
- 现在执行 `bash <(curl -fsSL .../install.sh)` 会先安装/刷新 `speed`，再进入主菜单。
...
```bash
speed                  # 无续跑状态打开控制台；有续跑状态自动继续当前流程
speed --version        # 查看版本
