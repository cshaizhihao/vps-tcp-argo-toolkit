...
`speed` 会自动识别续跑状态并继续完成。`speed --continue` 仍保留为兼容命令，但不再是推荐用法。
...
脚本启动会显示彩色 `SPEED SLAYER` ASCII 艺术字，并使用统一的彩色状态输出。Argo 原生安装阶段不再输出上游脚本瀑布，而是显示真实阶段进度：
...
Argo 部分已经改为 **Speed Slayer 原生安装链路**，不再调用 ArgoX 全家桶安装逻辑。

当前由 Speed Slayer 自己完成：
...
scripts/
  ...
  upstream/
    ...
    argox.sh                      # 历史上游脚本归档，仅供审计参考

docs/
  argox-vmess-ws-callchain.md     # 历史审计：用于解释为何剥离 ArgoX
```
...
ArgoX 仅作为历史上游来源和审计参考；Speed Slayer 当前安装链路不再调用它。
